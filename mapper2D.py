...
# Save index-to-name mapping for Mapper lookup
index_to_name = df['Name'].reset_index(drop=True)
print(index_to_name[42])  # replace 42 with the number shown on hover
...
# Step 3: Save knot names BEFORE dropping the Name column
# This aligns index-for-index with X and feature_df
knot_names = df['Name'].reset_index(drop=True).values  # e.g. ["3_1", "4_1", ...]

# Step 4: Check that Genus-4D exists as a column for coloring
color_col = 'Genus-4D'
...
# Step 5: Build the Mapper graph
cubeslist = [15, 20, 25]
overlaplist = [25, 50, 60, 75]
...
for q in cubeslist:
    for a in overlaplist:
        graph = mapper.map(
            lens,
            X,
            cover=km.Cover(n_cubes=q, perc_overlap=(a/100)),
            clusterer=KMeans(n_clusters=2, random_state=0, n_init=10)
        )

        # Step 6: Build custom tooltips array — one entry per data point
        # Each entry is an HTML string shown when you hover over a node member
        custom_tooltips = np.array([
            f"<b>{name}</b><br>{color_col}: {color_values[i]:.3f}"
            for i, name in enumerate(knot_names)
        ])

        # Step 7: Visualize and save as interactive HTML
        html_path = os.path.join(OUTPUT_DIR, f'mapper_graph_genus4D_ncubes{q}_povlp{a}.html')
        ...
        mapper.visualize(
            ...
            custom_tooltips=custom_tooltips,   # <-- This is the key change
        )
